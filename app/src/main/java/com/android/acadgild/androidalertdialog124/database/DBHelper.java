...
public class DBHelper {
    ...
    //To get Info Details
    public ArrayList<InfoData> getAllInfos() {
        ArrayList<InfoData> todos = new ArrayList<InfoData>();

        // select Info query
        String query = "SELECT  * FROM " + Constants.INFO_RECORD ;

        // get reference of the INFODB
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                todo.setKeyName(cursor.getString(1).toString());
                todo.setKeyPhone(cursor.getString(2).toString());
                todo.setKeyDob(cursor.getString(3).toString());
                todos.add(todo);
            } while (cursor.moveToNext());
        }
        Log.e("RET", "= " + todos.size());
        return todos;
    }
